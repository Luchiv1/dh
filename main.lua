--if getgenv().Executed == true then return end; getgenv().Executed = true
...
PCR_1.Name = "PCR_1"
PCR_1.Parent = game:GetService("Players").LocalPlayer:WaitForChild("PlayerGui")
...
--// Cheat Settings
local CamLock = {Enabled = false, KeyBind = Enum.KeyCode.Q, Radius = 120, Prediction = 0.14, Part = "HumanoidRootPart", Resolver = false, Notification = false}
...
function getClosest()	
    local closestPlayer	
    local shortestDistance = CamLock.Radius
	
    for i, v in pairs(Players:GetPlayers()) do
        if v ~= LocalPlayer and v.Character and v.Character:FindFirstChild("Humanoid") and v.Character.Humanoid.Health ~= 0 and v.Character:FindFirstChild(CamLock.Part) then	
...
function CalculatedVelocity(T) 
    ...
    local TI = NT - CT
    local Velocity = DT / TI
    CP = NP
    CT = NT
    return Velocity
end
	
function Notify(T, D) StarterGui:SetCore("SendNotification", {Title = "phit.xd", Text = T, Duration = D}) end	

--// KeyBinds
UserInputService.InputBegan:Connect(function(KeyBind, GPE) if GPE then return end
    if KeyBind.KeyCode == CamLock.KeyBind and CamLock.Enabled then
		Locked = not Locked
		if Locked then
			Victim = getClosest()
		else
...
RunService.Stepped:Connect(function()
    if CamLock.Enabled and Victim ~= nil then
		if CamLock.Resolver then	
			local Velocity = CalculatedVelocity(Victim)
			Camera.CFrame = CFrame.new(Camera.CFrame.p, Victim.Character[CamLock.Part].Position + Velocity * CamLock.Prediction)	
		else
			Camera.CFrame = CFrame.new(Camera.CFrame.p, Victim.Character[CamLock.Part].Position + Victim.Character[CamLock.Part].Velocity * CamLock.Prediction)
		end
    end
end)
